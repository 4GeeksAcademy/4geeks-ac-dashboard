const { PIPELINE_REGION_MAP, DEAL_FIELD_MAP, CONTACT_FIELD_MAP, DEAL_STATUS, classifyEngagementScore } = require('./config');
...
function ownerName(usersById, id) {
  if (!id) return null;
  const u = usersById && usersById.get(String(id));
  if (!u) return `User #${id}`;
  return [u.firstName, u.lastName].filter(Boolean).join(' ') || u.username || u.email || `User #${id}`;
}
function stageName(stagesById, id) {
  if (!id) return null;
  const st = stagesById && stagesById.get(String(id));
  return st ? (st.title || st.name || String(id)) : String(id);
}
// Deal Quality is the engagement score (see ENGAGEMENT_SCORE_BANDS). Bucket
// it so it can be filtered on like a category.
function scoreBandFor(v) {
  const b = classifyEngagementScore(v);
...
*/
// Side-loaded (?include=dealCustomFieldData) and bulk-listed rows have been
// seen with DIFFERENT key names for the same thing: `dealId` vs `deal`, and
// `customFieldId` vs `dealCustomFieldMetumId`/`dealCustomFieldMetum`. Reading
// only `row.dealId` silently dropped EVERY deal custom field in production
// (lost reasons, deal quality, feedback, won/lost dates were all null), which
// is why the Lost-reason filter only ever showed "All". Read all variants.
function dealRowDealId(row) {
  const v = row.dealId ?? row.deal ?? row.deal_id ?? row.dealid;
  return v == null ? null : String(typeof v === 'object' ? v.id : v);
}
function dealRowFieldId(row) {
  const v = row.customFieldId ?? row.custom_field_id ?? row.dealCustomFieldMetumId
    ?? row.dealCustomFieldMetum ?? row.dealCustomFieldMeta ?? row.field ?? row.fieldId;
  return v == null ? null : String(typeof v === 'object' ? v.id : v);
}
function dealRowValue(row) {
  // Live side-loaded rows carry ALL typed columns at once (text, text_blob,
  // number, date, currency) with the unused ones empty, so take the first
  // NON-EMPTY one -- `??` alone stopped at an empty text column and dropped
  // number fields (Deal Quality) and long-text fields (Client Comments).
  const candidates = [
    row.fieldValue, row.field_value, row.value,
    row.custom_field_text_value, row.custom_field_text_blob,
    row.custom_field_number_value, row.custom_field_date_value,
    row.custom_field_currency_value,
  ];
...
function buildDataset({ deals, dealCustomFieldData, contactsById, contactFieldValuesById, usersById, stagesById }) {
  const dealFieldsByDeal = new Map();
  (dealCustomFieldData || []).forEach((row) => {
    const key = dealRowDealId(row);
    const fid = dealRowFieldId(row);
    if (!key || !fid) return;
    if (!dealFieldsByDeal.has(key)) dealFieldsByDeal.set(key, {});
    dealFieldsByDeal.get(key)[fid] = dealRowValue(row);
  });
...
(deals || []).forEach((deal) => {
  ...
  const date = (deal.cdate || '').slice(0, 10);
                      ...
                      records.push({
                        ...
                        assignTo: ownerName(usersById, deal.owner),
                        stage: stageName(stagesById, deal.stage),
                        bucket,
reason: bucket === 'Won' ? 'Enrolled' : bucket === 'Active / Other' ? 'Open / in pipeline' : (lostReasons.length ? lostReasons.join(', ') : 'No reason recorded'),
                        ...
                        scoreBand: scoreBandFor(getDealField(deal.id, DEAL_FIELD_MAP.dealQuality)),
...
module.exports = { buildDataset, summarize, groupBy, isoWeekStart, dealRowDealId, dealRowFieldId, dealRowValue };
