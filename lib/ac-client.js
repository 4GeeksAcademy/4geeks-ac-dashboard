...
// Marks AC calls made on behalf of a person waiting on screen (opening a
// lead). They get their own pacing lane instead of queueing behind the
// background refresh -- which runs at 1 req/s nearly continuously, so a lead
// needing ~15 calls used to wait ~15s just for its turn.
const interactiveContext = new AsyncLocalStorage();
...
class ActiveCampaignClient {
  constructor({ apiUrl, apiKey, requestsPerSecond, timeoutMs } = {}) {
    ...
    this._nextSlot = 0;
    this._nextInteractiveSlot = 0;
    this.interactiveRequestsPerSecond = Number(process.env.AC_INTERACTIVE_RPS || 3);
...
// Run fn with every AC call inside it on the interactive lane.
interactive(fn) {
  return interactiveContext.run(true, fn);
}

async _throttle() {
  const now = Date.now();
  if (interactiveContext.getStore()) {
    const iv = 1000 / this.interactiveRequestsPerSecond;
    const islot = Math.max(now, this._nextInteractiveSlot);
    this._nextInteractiveSlot = islot + iv;
    const iwait = islot - now;
    if (iwait > 0) await new Promise((r) => setTimeout(r, iwait));
    return;
  }
...
async listUsers() {
  return this._paginateAll('/api/3/users', {}, 'users');
}
...
// ---- Scores ----
//
// AC "scores" are rules that award points for behaviour. The 4Geeks account
// has one built entirely from email opens and clicks ("Score: Quality +
// Engagement"), which makes it TRUE per-person engagement -- unlike campaign
// aggregates, which only say what a typical recipient did.
//
// Score definitions are few and change rarely, so they are cached once.
async listScores() {
  if (this.__scores) return this.__scores;
  const res = await this._getSafe('/api/3/scores', { limit: 100 });
  this.__scores = res.ok && Array.isArray(res.data?.scores) ? res.data.scores : [];
  return this.__scores;
}

// Score values attached to a deal (the probe showed these exist per-deal,
// carrying both contact and deal ids).
async getDealScoreValues(dealId) {
  const res = await this._getSafe(`/api/3/deals/${dealId}/scoreValues`, { limit: 100 });
  if (!res.ok) return [];
  return Array.isArray(res.data?.scoreValues) ? res.data.scoreValues : [];
}

async getContactScoreValues(contactId) {
  const res = await this._getSafe(`/api/3/contacts/${contactId}/scoreValues`, { limit: 100 });
  if (!res.ok) return [];
  return Array.isArray(res.data?.scoreValues) ? res.data.scoreValues : [];
}

// Resolves score values into named scores, so the dashboard can show
// "Score: Quality + Engagement = 500" rather than an opaque id.
async getNamedScores(contactId, dealId = null) {
  const [scores, dealValues, contactValues] = await Promise.all([
    this.listScores(),
    dealId ? this.getDealScoreValues(dealId) : Promise.resolve([]),
    this.getContactScoreValues(contactId),
  ]);

  const byId = new Map(scores.map((s) => [String(s.id), s]));
  const seen = new Map();

  [...dealValues, ...contactValues].forEach((v) => {
    const def = byId.get(String(v.score));
    const key = String(v.score);
    // Prefer the deal-scoped value when both exist.
    if (seen.has(key) && dealValues.includes(v) === false) return;
    seen.set(key, {
      scoreId: key,
      name: def?.name || `Score ${key}`,
      value: Number(v.scoreValue ?? 0),
      updatedAt: v.mdate || v.cdate || null,
      scopedTo: v.deal ? 'deal' : 'contact',
    });
  });

  const list = [...seen.values()].sort((a, b) => b.value - a.value);

  // Flag the open/click-driven score and attach its interpretation, so
  // callers get "High engagement - clicked a link" rather than a bare number.
  const { ENGAGEMENT_SCORE_NAME_PATTERN, classifyEngagementScore } = require('./config');
  list.forEach((s) => {
    if (ENGAGEMENT_SCORE_NAME_PATTERN.test(s.name)) {
      s.isEngagementScore = true;
      s.interpretation = classifyEngagementScore(s.value);
    }
  });
...
// Campaign metadata cache. Campaigns are few (~3,090) relative to sends
// (~5M) and their stats change slowly, so caching them makes per-lead
// engagement essentially free after the first lookup.
_campaignCache() {
  if (!this.__campaigns) this.__campaigns = new Map();
  return this.__campaigns;
}

async getCampaign(campaignId) {
  const cache = this._campaignCache();
  const key = String(campaignId);
  if (cache.has(key)) return cache.get(key);
  const res = await this._getSafe(`/api/3/campaigns/${campaignId}`);
  const campaign = res.ok ? (res.data?.campaign || null) : null;
  cache.set(key, campaign);
  return campaign;
}

// Per-contact email engagement, built the only way AC actually allows.
//
// PROVEN BY PROBE (do not "simplify" this back to contact sub-resources):
//   - /api/3/contacts/{id}/contactLogs        -> always 0 rows
//   - /api/3/contacts/{id}/trackingLogs       -> always 0 rows
//   - /api/3/logs?filters[contact]=           -> IGNORED, returns other contacts
//   - /api/3/logs?filters[subscriberid]=      -> WORKS, correctly scoped
//
// The send log says WHAT was sent, not what was opened. Open/click counts
// live on the CAMPAIGN. So we read this contact's sends, then join campaign
// stats to characterise them. Campaign-level rates are not the same as
// "this person opened it" -- see engagementBasis in the result -- so the
// numbers are labelled honestly rather than presented as per-person truth.
async getContactEmailEngagementViaLogs(contactId, { maxCampaigns = 25 } = {}) {
  const engagement = {
    sent: 0,
    opened: 0,
    clicked: 0,
    ...
    sources: [],
    unavailable: false,
    engagementBasis: 'campaign-aggregate',
  };

  const res = await this._getSafe('/api/3/logs', {
    'filters[subscriberid]': contactId,
    limit: 100,
  });

  if (!res.ok) {
    engagement.unavailable = true;
    engagement.sources.push({ name: 'logs', status: 'error', error: res.error, count: 0 });
    return engagement;
  }

  const rows = Array.isArray(res.data?.logs) ? res.data.logs : [];
  // Guard against the filter silently regressing to "ignored": if rows come
  // back for other contacts, treat it as no data rather than reporting
  // somebody else's engagement on this lead.
  const mine = rows.filter((r) => String(r.subscriberid) === String(contactId));
  if (rows.length && !mine.length) {
    engagement.unavailable = true;
    engagement.sources.push({ name: 'logs', status: 'filter-not-respected', count: 0 });
    console.warn(`[AC] /api/3/logs filter returned ${rows.length} rows, none for contact ${contactId} -- filter no longer respected`);
    return engagement;
  }

  engagement.sources.push({ name: 'logs', status: 'ok', count: mine.length, total: res.data?.meta?.total ?? null });
  engagement.sent = mine.length;
  engagement.logs = mine;

  const campaignIds = [...new Set(mine.map((r) => r.campaignid).filter(Boolean))].slice(0, maxCampaigns);
  const campaigns = new Map();
  // Parallel (the limiter still paces them); campaigns are cached after the
  // first lookup so repeat leads cost nothing here.
  await this._mapWithConcurrency(campaignIds, 4, async (id) => {
    const c = await this.getCampaign(id);
    if (c) campaigns.set(String(id), c);
  });

  mine.forEach((row) => {
    const iso = ActiveCampaignClient._readDate(ActiveCampaignClient._lowerMap(row), ['tstamp', 'created_timestamp']);
    if (iso && (!engagement.lastEmailDate || new Date(iso) > new Date(engagement.lastEmailDate))) {
      engagement.lastEmailDate = iso;
    }
    const c = campaigns.get(String(row.campaignid));
    if (!c) return;

    const sendAmt = Number(c.send_amt || 0);
    const uniqueOpens = Number(c.uniqueopens || 0);
    const uniqueClicks = Number(c.uniquelinkclicks || 0);
    // Probability this recipient opened, from the campaign's own rate.
    const openLikelihood = sendAmt > 0 ? uniqueOpens / sendAmt : 0;
    const clickLikelihood = sendAmt > 0 ? uniqueClicks / sendAmt : 0;

    if (openLikelihood >= 0.5) engagement.opened += 1;
    if (clickLikelihood >= 0.5) engagement.clicked += 1;

    engagement.events.push({
      date: iso ? iso.slice(0, 10) : null,
      type: 'email_sent',
      detail: c.name ? `Sent: ${c.name}` : 'Campaign email sent',
      campaignOpenRate: sendAmt ? Number((openLikelihood * 100).toFixed(1)) : null,
      campaignClickRate: sendAmt ? Number((clickLikelihood * 100).toFixed(1)) : null,
  ...
  console.log(`[AC] engagement(logs) contact=${contactId} sent=${engagement.sent} campaigns=${campaigns.size}`);
  return engagement;
}
...
//
// Tries the send log first (the only per-contact query AC actually honours),
// then falls back to the contact sub-resources for accounts where those do
// return data.
async getContactEmailEngagement(contactId) {
  const viaLogs = await this.getContactEmailEngagementViaLogs(contactId);
  if (!viaLogs.unavailable && viaLogs.sent > 0) return viaLogs;
  const viaSubResources = await this._getContactEmailEngagementViaSubResources(contactId);
  // Prefer whichever actually found sends.
  if (viaSubResources.sent > 0) return viaSubResources;
  return viaLogs.unavailable ? viaSubResources : viaLogs;
}

async _getContactEmailEngagementViaSubResources(contactId) {
  const engagement = {
    sent: 0,
    opened: 0,
    clicked: 0,
